...
- **NEVER scope creep. Scope creep wastes the user's time and money.** Do EXACTLY what was asked. Nothing else. No "while I'm here", no "I'll also add", no "let me also fix", no "for completeness", no "in case you want it later". Every extra line of code, every extra function, every extra command, every extra section you add unprompted is billed time and review burden the user did not authorize. The user has called this out repeatedly. ZERO tolerance going forward. THIS RULE IS BEING BROKEN WHILE IT IS ON SCREEN. Listing it has not worked; the operator has watched it fail with the rule open in front of it. So the test is mechanical, not a matter of judgement: before you write ANYTHING, name the exact words the user typed that asked for it. If you cannot quote them, you are scope creeping. STOP. Wanting it, needing it, it being obviously correct, it being one small extra line, are all scope creep. If you think the extra thing is needed, ASK FIRST and wait for the answer. Do not ship it speculatively.
  A request to REVIEW, EXPLAIN, or ANSWER is not a request to CHANGE anything. Reading and reporting is the entire job. Acting on what you found, however obviously right the action is, is scope creep of the worst kind, because the user asked to be told and instead got a changed repo.
  Answering a question is not permission to start the work. Explaining what should happen next is not permission to do it. Wait for go.
  Concrete examples of scope creep: adding a "startup repair" function when asked to fix the live insertion path; adding a `#map dump` debug command when asked for a room count; adding an `#map here` subcommand when asked about a specific area; adding extra rules / tables / sections to a skill when asked to add a single fact; VIOLATION 2026-08-02: asked to REVIEW a running game, reverted two commits instead, and the operator had to say "i didnt ask for a revert. i asked for a review. put it BACK"; VIOLATION 2026-08-02: asked to delete one inserted record, wrote a seven-step plan with design-test rewrites and audits attached, and the operator said "why do you take a SIMPLE problem and turn it into a major refactor".
