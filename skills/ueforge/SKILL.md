...
## Repo layout (framework only)

```
ueforge/
  README.md
  docs/                       # READ FIRST when working on ueforge
    architecture.md             # the contract (composition + k8s + compliance)
    PERFORMANCE.md, RESEARCH.md
    lifecycle.md, ue-sdk.md, hooks.md, pe-queue.md, counters.md
    rpg.md, http.md, imgui.md, settings.md, worker.md
    data-table.md, memory-tools.md, status-effects.md
    ue-engine.md, native.md, uasset.md, mod-formats.md
    inspection.md, rust-port.md, ue4ss-port.md
  cpp/
    ueforge_shim.cpp, ueforge_cppusermodbase.hpp,
    ueforge_imgui_bridge.hpp, ueforge_ui.cpp
    imgui/                      # vendored as git submodule
  ue4ss/UE4SS.lib
  src/
    lib.rs, mod_main.rs (ModDef + ue4ss_mod!), build.rs
    log.rs, settings.rs (with watch / hot-reload), counters.rs,
    ring.rs, hex.rs, args.rs, envelope.rs, server.rs,
    pe_queue.rs, selector.rs, ops.rs (OP_REGISTRY),
    shutdown.rs (SHUTDOWN_REGISTRY), scanner.rs,
    hot_reload.rs, hook/, ue/
    discovery.rs                # cache + describe_* lazy
    data_table.rs               # DataTableDef + FieldTweak + NamedFieldTweak + ClassNamedFieldTweak
    dynamic_tweaks.rs           # apply_all_when_ready per-table on_first_sight workers
    stacks.rs, difficulty.rs    # framework modules
    damage/, inventory/         # framework modules
    rpg/                        # SkillDef/EffectDef/TriggerDef/StatusDef + Tracker/SlotPoller/SlotStore/VanillaCache/DisabledSkills/Curve/PercentFormat
    uasset.rs                   # .uasset lib + CLI (replaces old python scripts)
    ui.rs                       # safe Rust ImGui wrappers
    ui_scanner.rs, ui_data_table_browser.rs,
    ui_class_browser.rs, ui_struct_browser.rs,
    ui_dynamic_tweaks.rs
    client/                     # test-side Api<S>, scenario DSL, diff, research, perf, timeseries, thread, cdo
    winproc.rs, parms.rs
    bin/                        # cargo deploy CLI
```
